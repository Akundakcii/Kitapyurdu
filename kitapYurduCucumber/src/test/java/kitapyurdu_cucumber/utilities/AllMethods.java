package kitapyurdu_cucumber.utilities;
...
import static kitapyurdu_cucumber.utilities.Driver.driver;
...
//        String script = "arguments[0].style.border='3px solid green';";
        ...
        for (int i = 1; i < sayfadakiUrunSayisi; i++) {
            if (i == number) {
                if (!addElement.get(i).isDisplayed()) {
                    AllMethods.scroll(addElement.get(i));
                }
                ...
                AllMethods.clickByJavaScript(addElement.get(i));
    ...
    }//Elemenet Locator bulma

    public static Boolean isDisplayed(By Locator) {
        return find(Locator).isDisplayed();
    } // Bu alan varmı dıye dondrudugumuz alan true/ False

    public static void resimEkleText(String text, By Locator) {
        find(Locator).sendKeys(text);
    }

    public static void tik(By Locator) {
        find(Locator).click();
